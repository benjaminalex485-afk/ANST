...
export function Sidebar() {
  ...
  // Keep Zustand activeTab in perfect sync with TanStack Router path
  const path = location.pathname;
  const computedTab = path === '/' ? 'dashboard' : path.replace('/', '');
  ...
  useEffect(() => {
    if (computedTab !== activeTab) {
      uiDispatch({
        ...
        type: 'ui:tab_changed',
        payload: { tab: computedTab },
      });
    }
  }, [computedTab, activeTab, uiDispatch]);

  const handleTabSelect = (tabId: string, to: string) => {
    uiDispatch({
      metadata: {
        version: 1,
        eventId: crypto.randomUUID(),
  ...
  return (
    ...
    >
      ...
      <nav className="flex-1 overflow-y-auto py-2">
        {navItems.map((item) => {
          const active = computedTab === item.id;
          const Icon = item.icon;
          return (
            <button
              key={item.id}
              onClick={() => handleTabSelect(item.id, item.to)}
              className={cn(
